...
- Do not act on issues with the label `kody:stuck`, `kody:no-auto-resume`, or `kody:stalled`.
...
6. Drop any issue with the labels `kody:stuck`, `kody:no-auto-resume`, or `kody:stalled`.
...
Garbage collection:
- Drop entries from `data.perIssue` whose issue is closed, has `core.status !== "running"`, or has the `kody:stuck` / `kody:no-auto-resume` label.
