...
describe('CSP Configuration - Vercel Feedback Script on /admin', () => {
  ...
  it('should include *.gravatar.com in img-src for /admin routes (covers www subdomain)', async () => {
    const configContent = fs.readFileSync(nextConfigPath, 'utf8')
    ...
    // Admin routes MUST have *.gravatar.com (not just gravatar.com) to match
    // https://www.gravatar.com/avatar/... URLs used by Payload user avatars.
    // Without the wildcard, CSP 'gravatar.com' does not match 'www.gravatar.com'.
