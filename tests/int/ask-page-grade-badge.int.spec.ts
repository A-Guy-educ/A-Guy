...
/**
 * Integration tests: AskConversationGrid renders courseLabel as grade badge
 *
 * Bug #1868: Grade badge on /ask page appears orphaned above the main
 * Questions & Answers content, visually disconnected from the page header.
 * The GradeSection is rendered as a sibling to <main>, not integrated into
 * the page header alongside other metadata.
 *
 * Fix: Integrate grade badge into the page header inside <main>, not as
 * a separate section above the main content.
 */
...
const GRADE_LEVEL = 'grade-8-ask-badge-test'
// courseLabel must match gradeLevel for queryChaptersByGrade to find the course
const COURSE_LABEL = GRADE_LEVEL
const COURSE_TITLE = 'Ask Test Course'
const TENANT_SLUG = `ask-badge-test-tenant-${Date.now()}`
...
beforeAll(async () => {
  ...
  const category = await payload.create({
    ...
    data: {
      title: 'Ask Badge Test Category',
      slug: `ask-badge-cat-${Date.now()}`,
    } as any,
    overrideAccess: true,
  })
  ...
  const course = await payload.create({
    ...
    data: {
      ...
      title: COURSE_TITLE,
      slug: `ask-badge-test-course-${Date.now()}`,
      status: 'published',
  ...
  const chapter = await payload.create({
    ...
    data: {
      course: course.id,
      title: 'Ask Badge Test Chapter',
      slug: `ask-badge-chapter-${Date.now()}`,
      status: 'published',
      isActive: true,
    } as any,
    overrideAccess: true,
  ...
  await payload.create({
    ...
    data: {
      ...
      slug: `ask-badge-practice-${Date.now()}`,
      status: 'published',
      isActive: true,
      type: 'practice',
      locale: 'he',
...
describe('prefetchStudyData — courseLabel for ask page grade badge', () => {
  it('returns the full courseLabel (not truncated) for the ask page grade badge', async () => {
    // Bug #1868: courseLabel should be the full string, not empty or truncated.
