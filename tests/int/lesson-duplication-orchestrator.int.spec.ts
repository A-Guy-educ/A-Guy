...
describe('Lesson duplication orchestrator — integration', () => {
  ...
  beforeAll(async () => {
    ...
    // Exercise 3 (index 2) must have a slug containing '-3' so the mock throws for it
    for (let i = 0; i < 5; i++) {
      const ex = await payload.create({
        collection: 'exercises',
        data: {
          title: `Orch Exercise ${i} ${ts}`,
          // Slug includes '-3' for index 2 so mock generateVariation throws on 3rd exercise
