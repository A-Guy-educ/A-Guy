...
/**
 * CSP Configuration Tests - Issue #1832
 *
 * Tests that Content-Security-Policy headers allow Gravatar images
 * to load on /admin routes.
 *
 * Bug: User avatar images from Gravatar (e.g., https://secure.gravatar.com/avatar/...)
 * are blocked on /admin because *.gravatar.com is not in the img-src directive.
 */
...
describe('CSP Configuration - Gravatar Images on /admin', () => {
  ...
  it('should include *.gravatar.com in img-src for /admin routes', async () => {
    ...
    // Admin routes MUST have *.gravatar.com in img-src for user avatars to work
