...
export const POST = withApiHandler<RetryBody, unknown>(
  ...
  async ({ body, request }) => {
    ...
    const oldFailures = [...(((record.failures as unknown[]) ?? []) as FailureEntry[])]
    ...
    // 6. Run the full single-exercise pipeline
    ...
    const { result, newFailures } = await processExercise(
