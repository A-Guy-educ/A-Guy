  ...
  switch (event.event_type) {
    case 'CHECKOUT.ORDER.APPROVED': {
      ...
      const transactions = await payload.find({
        ...
        where: {
          tranzilaTransactionId: { equals: orderId },
        },
        limit: 1,
        depth: 0,
        overrideAccess: true,
    ...
    case 'PAYMENT.CAPTURE.COMPLETED': {
      ...
      // Idempotency guard: only process if still pending (skip if already completed, refunded, or failed)
    ...
    case 'PAYMENT.CAPTURE.REFUNDED': {
      ...
      const transactions = await payload.find({
        collection: 'transactions',
        where: {
          tranzilaTransactionId: { equals: captureId },
        },
        limit: 1,
        depth: 0,
        overrideAccess: true,
