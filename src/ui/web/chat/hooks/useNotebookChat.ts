...
interface UseNotebookChatProps {
  ...
  chapterId?: string
  courseId?: string
  // Active block ID for multi-question exercises (tracks which question student is working on)
...
export function useNotebookChat({
  ...
  const sendContextualHelp = async (prompt: string) => {
    ...
    setIsLoading(true)
    const context = { exerciseId, lessonId, chapterId, courseId, categoryId, activeBlockId }
