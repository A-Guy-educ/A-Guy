...
/**
 * Fill missing `solution` fields on question blocks using AI generation.
 *
 * Walks the blocks array and, for each question block that has no `solution`
 * value (either because the LaTeX-derived solution didn't have a matching
 * sub-question label or there was no solution at all), calls the same
 * `generateSupport` service the admin "Generate with AI" button uses.
 *
 * Mutates `blocks` in place. Failures are logged and the block is left
 * without a solution rather than aborting the conversion.
 */
export async function fillMissingSolutionsWithAI(
  ...
  // Find question blocks that don't already have a solution
  const targets: number[] = []
  for (let i = 0; i < blocks.length; i++) {
    if (!isQuestionBlock(blocks[i])) continue
    const b = blocks[i] as ContentBlock & { solution?: InlineRichText }
    if (b.solution?.value) continue
    targets.push(i)
  }
  ...
  reqLogger.info(
    { missingSolutions: targets.length, totalBlocks: blocks.length },
    'Filling missing solutions with AI',
  )

  // Generate sequentially to avoid overwhelming the LLM API / hitting rate limits.
  // Each call is one Gemini request.
  for (const idx of targets) {
    ...
    try {
      ...
      if (result.success && result.data?.solution) {
        ...
        reqLogger.warn(
          { blockId: block.id, error: result.error },
          'AI solution generation failed for block — leaving solution empty',
        )
      }
    } catch (err) {
      reqLogger.warn(
        { blockId: block.id, error: err instanceof Error ? err.message : 'Unknown error' },
        'AI solution generation threw — leaving solution empty',
      )
    }
  }
}
