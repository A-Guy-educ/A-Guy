...
- Deliverable: code + tests (no planning-only output)
 Problem
Plan is still computed/rendered/persisted automatically while editing date/topics/mastery.
 Required Behavior
- Before explicit Generate click:
  - show empty state (`מוכנים לצאת לדרך?`)
  - show CTA (`צור תוכנית לימודים` / `Generate Example Plan`)
...
- After plan exists:
 ...
 Visual Direction
Follow `test plan.html` for layout/style vibe (RTL, Assistant, Tailwind cards/timeline).
...
- `src/app/(frontend)/study-plan/_components/DayCard.tsx` (if needed)
 Acceptance Criteria
