...
# Adaptive Practice Schedule (7-Day Tactical Planner)

---

# 1. Product Definition

Adaptive Practice Schedule is a deterministic weekly planning engine that generates a 7-day forward study plan based on:
...
- Generates exactly 7 days forward from today
- Stores the plan as a single active snapshot per course
- Recalculates only future (non-completed) days when inputs change
- Is fully deterministic (same input → same output)

This is a Weekly Tactical Planner (not long-term planning).
...
- One active snapshot per course
- Snapshot contains:
  - examDate
  - topics
  - days[7]
...
- Use deterministic weighted cycling
...
Weighted deterministic cycling required.
...
- Recalculate only future planned days
- Maintain 7 total days

---

# 5. Integration

Each day has primary CTA:

- practice → Start Practice
- hybrid / full_simulation → Start Simulation
- reinforcement → Log Mistakes
- warmup → Start Review

Route receives:
- dayId

System resolves full context from snapshot.

---

# 6. Edge Cases

- No topics → do not generate plan
- Single topic → vary activity type across days
- Exam tomorrow → warmup only
- Topic deleted → remove from future days only
...
# 9. HTML Template (Base UI Layout)

Below is the base static HTML layout for the 7-day planner.
Logic injection should populate day cards dynamically.
