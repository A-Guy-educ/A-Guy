...
Before building: seed the required data.

Create 5 Prompts entries for teacher profiles (raw text), and 5 TeacherProfiles entries that reference those Prompts via systemPrompt relationship.

Use these slugs:
teacher_strict, teacher_thorough, teacher_patient, teacher_focused (default), teacher_challenging

Use the following prompt texts (store in Prompts.template or equivalent raw text field):

🔹 Strict Teacher (in a good way)
...
Then proceed to implement the feature end-to-end and open a PR.
