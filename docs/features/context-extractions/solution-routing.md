...
### Phase 5: AI fallback for missing solutions

After Phase 4, any question block that still has no `solution` field gets one generated by AI. This handles cases where:
...
`fillMissingSolutionsWithAI()` calls the same `generateSupport` service the admin "Generate with AI" button uses ([useGenerateSupport.ts](../../../src/ui/admin/ExerciseContentEditor/editors/useGenerateSupport.ts)). One AI call per missing solution, processed sequentially to respect rate limits.

**Failure handling**: if AI generation fails (rate limit, network error, parse error), the block is left without a solution and a warning is logged. The conversion continues — failures don't abort the whole operation.
